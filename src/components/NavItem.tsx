  ...
  const renderMainItem = (item: NavItemType) => {
    return (
      <Link
        ...
        className="inline-flex items-center px-4 py-2 text-lg hover:text-primary xl:px-5 "
        href={item.href || '/'}
      >
        {item.name}
        {item.type && (
  ...
  const renderDropdownMenuNavlink = (item: NavItemType) => {
    return (
      <Link
        ...
        rel="noopener noreferrer"
        className="hover flex items-center rounded-md px-4 py-2 font-normal hover:bg-primary/10"
        href={item.href || ''}
      >
        {item.name}
        {item.type && (
        ...
        {() => (
          <>
            ...
            >
              ...
              >
                <ul className="ring-opacity/5 relative grid space-y-1 rounded-lg bg-white py-4 shadow-lg ring-1 ring-black dark:bg-neutral-900">
                  {item.children?.map((i) => {
                    if (i.type) {
