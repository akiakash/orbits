...
const NavMobile: React.FC<NavMobileProps> = ({ onClickClose }) => {
      ...
      <ul className="nav-mobile-sub-menu pb-1 pl-6 text-base">
        {item.children?.map((i) => (
          <Disclosure key={i.id} as="li">
            ...
            {i.children && (
              <Disclosure.Panel>
                {renderMenuChild(i, 'pl-3 text-slate-600 dark:text-slate-400 ')}
              </Disclosure.Panel>
            )}
          </Disclosure>
        ))}
  ...
  const renderItem = (item: NavItemType, index: number) => {
    return (
      <Disclosure key={index} as="li">
        <Link
          className="flex w-full items-center rounded-lg px-4 text-sm font-medium uppercase tracking-wide hover:bg-slate-100 dark:hover:bg-slate-800"
          href={{
  ...
  return (
    <div className="h-screen w-full divide-y divide-neutral-300 overflow-y-auto bg-white py-2 shadow-lg ring-1 transition dark:divide-neutral-700 dark:bg-neutral-900">
